...
export default class GenericFileDAO {
  ...
  async save(object) {
    ...
    writeFile(this.fileName, JSON.stringify(objectsAux, null, 4), "utf-8")
      .then(() => {
  ...
  async getAll() {
    try {
      const archivo = await readFile(this.fileName, "utf-8");
  ...
  async deleteAll() {
    try {
      await writeFile(this.fileName, JSON.stringify([]), "utf-8");
